...
function loadActList(event) {
  actList.options.length = 0;
    const selectedValue = categoryList.value;

  for (const activity of activities) {
    if (activity.category == selectedValue) {
      addOptionToList(
        activity.category,
        activity.name,
        activity.id,
...
function addOptionToList(text, value, description, id, location, price, list) {
